...
import type { Client, UserTenant } from "@/api";
...
export function useClients() {
  ...
  const addClient = async (data: { name: string; email: string; phone?: string; status: string; tenant: number }) => {
    const token = getToken();
  ...
  const editClient = async (id: number, data: Partial<{ name: string; email: string; phone?: string; status: string; tenant: number }>) => {
    const token = getToken();
    if (!token) return;
